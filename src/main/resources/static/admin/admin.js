...
async function saveAttraction(event) {
    ...
    const attractionData = sanitizeAttractionPayload({
        name: normalizeTextOrFallback(document.getElementById('attractionName').value, currentAttraction?.name ?? null),
        type: normalizeTextOrFallback(document.getElementById('attractionType').value, currentAttraction?.type ?? null),
...
async function deleteZone(id) {
    const confirmed = window.confirm('Estas seguro de eliminar esta zona? Sus atracciones seran reasignadas automaticamente.');
    if (!confirmed) {
        return;
    }
    ...
    try {
        await apiFetch(`/attractions/zones/${id}`, { method: 'DELETE' });
        zones = zones.filter((zone) => Number(zone.id) !== Number(id));
        const zoneCard = document.getElementById(`zone-card-${id}`);
        if (zoneCard) {
            zoneCard.remove();
        }
        if (Array.isArray(graphSnapshot.zones)) {
            graphSnapshot = {
                ...graphSnapshot,
                zones: graphSnapshot.zones.filter((zone) => Number(zone.id) !== Number(id))
            };
        }
        renderZonesGrid();
        populateZoneSelectors();
        ...
        showAlert('Zona eliminada exitosamente', 'success');
        await loadZones();
        await Promise.all([loadAttractions(), loadGraph(), loadDashboard(), loadOperators()]);
    } catch (error) {
        showAlert(`No fue posible eliminar la zona: ${error.message}`, 'danger');
    }
}
